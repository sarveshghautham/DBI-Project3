...
//Overloading the operators such that the OrderMaker objects are accessed through the streams
ostream& operator<<(std::ostream& os, const OrderMaker& myorder) {
  os << myorder.numAtts << ' ';
  for(int i=0; i < myorder.numAtts; ++i) 
	os << myorder.whichAtts[i] << ' ';
  for(int i=0; i < myorder.numAtts; ++i) 
	os << myorder.whichTypes[i] << ' ';
  os << endl;
  return os;
}

//Overloading the operators such that the OrderMaker objects are accessed through the streams
istream& operator>>(std::istream& is, OrderMaker& myorder) {
  is >> myorder.numAtts;
  for(int i=0; i < myorder.numAtts; ++i) 
